...
def adaptiveThresholding(torch_img, upscale):
    ...
    if upscale:
        torch_img = ((torch_img - torch_img.min()) / (torch_img.max() - torch_img.min()) * 255)
...
def get_occlusion_gt(gt, images, sl_images):
    """
    Returns a prediction of the occluded images without the occlusions.
    So they only contain shadow and light.
    ...
    for i in range(images.shape[1]):
        ...
        for j in range(sl_images.shape[1]):
            ...
            sl = sl_images[:, j, :, :].unsqueeze(0).type(torch.FloatTensor) # sl.mean() ~ 0.5 (or in that order of magnitude)!!
